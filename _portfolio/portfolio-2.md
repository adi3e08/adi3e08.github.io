...
\\[
\overbrace{\textbf{M}(\textbf{q})}\_{\substack{\\\ \text{Mass} \\\ \\\ \text{Matrix}}} \, \ddot{\textbf{q}} + \overbrace{\textbf{C}(\textbf{q},\dot{\textbf{q}}) \, \dot{\textbf{q}}}\_{\substack{\\\ \tfrac{\partial }{\partial \textbf{q}} \big(\textbf{M}(\textbf{q})\, \dot{\textbf{q}} \big) \, \dot{\textbf{q}} - \tfrac{\partial }{\partial \textbf{q}} \big( \tfrac{1}{2} \, \dot{\textbf{q}}^{T} \, \textbf{M}(\textbf{q})\, \dot{\textbf{q}} \big) \\\ \\\ \text{Coriolis} \\\ \\\ \text{Term}}} + \overbrace{\textbf{G}(\textbf{q})}\_{\substack{\\\ \tfrac{\partial \mathcal{V}(\textbf{q})}{\partial \textbf{q}} \\\ \\\ \text{Gravitational} \\\ \\\ \text{Term}}} = \boldsymbol\tau
\\]
